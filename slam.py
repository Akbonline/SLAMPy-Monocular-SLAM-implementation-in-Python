...
class SLAM:
    ...
    def generate(self, image):
        ...
        for pt1, pt2 in zip(frame1.key_pts[x1], frame2.key_pts[x2]):
            ...
            # cv2.drawMarker(self.image, (point1), cc.red, 1, 5, 1, 8)
            cv2.circle(self.image, (u1, v1), color=(0, 0, 255), radius=2)
            cv2.line(self.image, (u1, v1), (u2, v2), (0, 255, 255), 1)
...
if __name__ == "__main__":
    ...
    # cap.set(cv2.CAP_PROP_POS_FRAMES, 152000)
    slam = SLAM()
    while cap.isOpened():
        ret, frame = cap.read()
        if ret == True:
